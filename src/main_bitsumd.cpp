...
int main(int argc, const char *argv[]) try {
	...
	if( !export_blocks.empty() && !backup_blockchain.empty() ){
		std::cout << "You can either export blocks or backup blockchain on one run of bitsumd" << std::endl;
	...
	platform::ExclusiveLock coin_lock(coin_folder, "bitsumd.lock");

	logging::LoggerManager log_manager;
	log_manager.configure_default(config.get_data_folder("logs"), "bitsumd-");
